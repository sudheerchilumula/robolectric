...
public class ShadowActivityManager {
  ...
  private final HashMap<OnUidImportanceListener, Integer> listenerToImportanceMap = new HashMap<>();
  ...
  protected void addOnUidImportanceListener(
    ...
    if (listenerToImportanceMap.containsKey(listener)) {
  ...
  @Implementation(minSdk = O)
  @HiddenApi
  protected void removeOnUidImportanceListener(OnUidImportanceListener listener) {
    if (!listenerToImportanceMap.containsKey(listener)) {
    ...
    listenerToImportanceMap.forEach(
        (listener, importanceCutpoint) -> {
          if ((newImportance - importanceCutpoint) * (oldImportance - importanceCutpoint) <= 0) {
            listener.onUidImportance(uid, newImportance);
          }
        });
  }
}
